...
class LegoDetectorNode(Node):
    def __init__(self):
        ...
        # Needs to be tuned according to arena lighting!
    ...
    def timer_callback(self):
        ...
        # Retrieve frames
        frame = cv2.flip(inRgb.getCvFrame(), -1) # -1 to flip image if camera is mounted upside down
        ...
        for cnt in contours:
            ...
            if 100 < area < 15000: # Size filter to ignore noise
                ...
                if d > 0 and d < 3.0: # Ignore errors and objects further than 3 meters
                    # Pixel to 3D Conversion (Standard camera frame: Z forward, X right, Y down)
                    px = (cx - 320) * d / 470.0
                    py = (cy - 240) * d / 470.0
                    pz = d
        ...
        for objID, data in tracked.items():
            ...
            # Publish global position on the map (optional, useful for RViz debugging)
            ...
            try:
                ...
                pass # Silently ignore if TF map is not yet available
