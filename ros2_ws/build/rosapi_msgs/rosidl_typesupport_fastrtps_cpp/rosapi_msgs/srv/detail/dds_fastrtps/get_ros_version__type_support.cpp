  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = rosapi_msgs::srv::GetROSVersion_Request;
    is_plain =
      (
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = rosapi_msgs::srv::GetROSVersion_Response;
    is_plain =
      (
      offsetof(DataType, distro) +
      last_member_size
      ) == ret_val;
  }
...
static rosidl_service_type_support_t _GetROSVersion__handle = {
  rosidl_typesupport_fastrtps_cpp::typesupport_identifier,
