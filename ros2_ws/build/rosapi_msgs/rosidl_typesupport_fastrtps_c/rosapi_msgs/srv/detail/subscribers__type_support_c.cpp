  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = rosapi_msgs__srv__Subscribers_Request;
    is_plain =
      (
...
static size_t _Subscribers_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_rosapi_msgs__srv__Subscribers_Request(
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: subscribers
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->subscribers, size)) {
      fprintf(stderr, "failed to create array for field 'subscribers'");
      return false;
    }
    auto array_ptr = ros_message->subscribers.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = rosapi_msgs__srv__Subscribers_Response;
    is_plain =
      (
...
static size_t _Subscribers_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_rosapi_msgs__srv__Subscribers_Response(
    full_bounded, is_plain, 0);
