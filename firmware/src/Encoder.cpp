...
// GPIO 34-39 are input only with no pull resistors; skip them.
// All current encoder pins are < 34 so each gets an internal pull-up.
...
esp_err_t Encoder::init()
{
    ...
    // Glitch filter: reject spikes under 1 us on encoder lines (motor/PWM noise)
    // that would cause false counts while the wheel is still.
    ...
    //   A rising B=H: +1  A rising B=L: -1
    //   A falling B=H: -1  A falling B=L: +1
    ...
    //   B rising A=H: -1  B rising A=L: +1
    //   B falling A=H: +1  B falling A=L: -1
    ...
    // Watch points required: accum_count only triggers at limit crossings.
    // Without them, the counter resets silently at +/-30000 and ticks are lost.
